...
namespace TimeUtils
{
    ...
    inline bool isSameLodalHour(time_t t1, time_t t2)
    {
        struct tm tv1;
        struct tm tv2;
        getLocalTime(tv1, t1);
        ...
        return (
            tv1.tm_year == tv2.tm_year && tv1.tm_mon == tv2.tm_mon && tv1.tm_mday == tv2.tm_mday && tv1.tm_hour == tv2.tm_hour);
    }

    inline bool isSameLodalDay(time_t t1, time_t t2)
    {
        return localDaysFromEpoch(t1) == localDaysFromEpoch(t2);
    }

    inline bool isSameLodalWeek(time_t t1, time_t t2)
    {
        return localWeeksFromEpoch(t1) == localWeeksFromEpoch(t2);
    }

    inline bool isSameLodalMonth(time_t t1, time_t t2)
    {
        struct tm tv1;
        struct tm tv2;
        getLocalTime(tv1, t1);
    ...
    inline bool isSameLodalYear(time_t t1, time_t t2)
    {
        struct tm tv1;
        struct tm tv2;
        getLocalTime(tv1, t1);
